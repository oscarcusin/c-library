...
// Returns the shortest path from FROM to TO in the weighted graph WG, using Dijkstra's algorithm.
// The distance from FROM to TO is stored in DIST, and the previous vertex in the path is stored in PREV.
// Unreachable vertices have a distance of LONG_MAX.
size_t shortest_path(weighted_graph * wg, size_t from, long dist[], size_t prev[]);

// Returns the shortest path from FROM to TO in the weighted graph WG, using Bellman-Ford's algorithm.
// The distance from FROM to TO is stored in DIST, and the previous vertex in the path is stored in PREV.
// Unreachable vertices have a distance of LONG_MAX.
...
// Finds the shortest path between every pair of vertices in the weighted graph WG.
